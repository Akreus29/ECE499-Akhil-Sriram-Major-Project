...
Usage:
  python scripts/kcf_demo_visual.py             # interactive
  python scripts/kcf_demo_visual.py --export    # save 6 PNGs non-interactively
"""
...
# Simulated inter-frame shift for Frame 3 (pixels)
FRAME3_SHIFT = (8, 7)  # (row_shift, col_shift)

# Export mode: predefined click (matches the demo run shown in presentation)
EXPORT_CLICK_R = 213
EXPORT_CLICK_C = 305
EXPORT_OUT_DIR = os.path.join(ROOT_DIR, 'docs', 'MidTerm Report')


def save_step(fig, name):
    path = os.path.join(EXPORT_OUT_DIR, name)
...
def wait(fig, msg='', export_name=None):
    if EXPORT_MODE:
        if export_name:
            save_step(fig, export_name)
        return
    if msg:
        print(f'  >> {msg}')
...
def main():
    ...
    fig.text(0.795, 0.50,
             ...
             'the target centre.\n\n'
             f'KCF will extract a {N}x{N}\n'
             'patch and learn a filter\n'
             'from it.\n\n'
             '(32x32 patch)',
    ...
    # Save Step 1 initial state
    if EXPORT_MODE:
        save_step(fig, 'demo_s1_select.png')

    # Get click (interactive or preset)
    if EXPORT_MODE:
        click_x, click_y = float(EXPORT_CLICK_C), float(EXPORT_CLICK_R)
        print(f'  [export] Using preset click at ({EXPORT_CLICK_R}, {EXPORT_CLICK_C})')
    else:
    ...
    # Save Step 2 state (target selected + "Training KCF..." panel)
    if EXPORT_MODE:
        save_step(fig, 'demo_s2_trained.png')
    ...
    fig.canvas.draw()
    wait(fig, 'Step 4: NCC found target in Frame 2', export_name='demo_s3_ncc.png')
    ...
    fig.canvas.draw()
    wait(fig, 'Step 5: Switching to KCF tracking...', export_name='demo_s4_handoff.png')
    ...
    fig.canvas.draw()
    wait(fig, 'Step 6: KCF-only tracking on Frame 3', export_name='demo_s5_kcf.png')
    ...
    fig.text(0.795, 0.50,
             ...
             f'  KCF ONLY (no NCC)\n'
             f'  Window at: ({tl2_r + half}, {tl2_c + half})\n'
             f'  KCF peak:  ({d3_r:+d}, {d3_c:+d})\n'
             f'  Tracked to: ({c3_r}, {c3_c})',
             fontsize=10, color='white', ha='center', va='center',
             fontfamily='monospace',
    ...
    fig.text(0.32, 0.93,
             f'NCC Detection + KCF Tracking Demo  |  KCF displacement: ({d3_r:+d}, {d3_c:+d})',
    ...
    if EXPORT_MODE:
        save_step(fig, 'demo_s6_summary.png')
        print('\n  >> Export complete. 6 demo images saved.')
        plt.close(fig)
    else:
